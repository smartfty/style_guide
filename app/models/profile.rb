    ...
    RLayout::Container.new(width:158.74015748031,  height: 75) do
      ...
      container(x: 0, y: 20, width:100, bottom_margin: 10, fill_color: 'clear') do
        <% if name && work && work != "" && position && position != "" %>
          <% if name.include?('-') %>
          text('<%= name.split("-").first.gsub("+", " ") %>', text_alignment: 'right', from_right: 10, y:17, font: 'KoPubDotumPB', font_size: 9, text_color:"CMYK=0,0,0,10", fill_color: 'clear')
          <% else  %>
          text('<%= name.gsub("+", " ") %>', text_alignment: 'right', from_right: 10, y:17, font: 'KoPubDotumPB', font_size: 9,text_color:"CMYK=0,0,0,10", fill_color: 'clear')
          <% end  %>
          text('<%= work %>', text_alignment: 'right', from_right: 10, y:30, font: 'KoPubDotumPL', font_size: 8, text_color:"CMYK=0,0,0,10", fill_color: 'clear')
          text('<%= position %>', text_alignment: 'right', from_right: 10, y:41, font: 'KoPubDotumPL', font_size: 8, text_color:"CMYK=0,0,0,10", fill_color: 'clear')
        <% elsif position == "" || position == nil %>
          text('<%= name.gsub("+", " ") %>', text_alignment: 'right', from_right: 10, y:28, font: 'KoPubDotumPB', font_size: 9, text_color:"CMYK=0,0,0,10", fill_color: 'clear')
