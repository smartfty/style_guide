  ...
  def svg_box
    ...
    eval_layout.each do |box|
      ...
      if box.length == 5
        ...
        string += "<text x='#{box[0]*svg_unit_width + 3}'y='#{box[1]*svg_unit_height + 12}' stroke-width='0' class='small' fill='skyblue'>#{char_count}</text>"
        if box[2] >3 && box[3]>3
          char_count = number_of_chars(box[2], box[3], true)
          string += "<text x='#{box[0]*svg_unit_width + 3}'y='#{box[1]*svg_unit_height + 22}' stroke-width='0' class='small'>#{char_count}(사진)</text>"
        end
       end
    end
    string
