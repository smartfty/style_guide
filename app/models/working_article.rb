...
class WorkingArticle < ApplicationRecord
  ...
  def image_source
    if page_number == 22
      ...
      system("cd #{issue.path}/images/ && sip -s format jpeg #{graphic_file_name} --out #{newsml_issue_path}/#{@photo_item}")
  ...
  def story_xml # 내일닷컴(데스크탑용) 기사 xml 생성
    ...
    @sub_head_line  = subtitle.gsub("\r\n", "]]></SubHeadLine><SubHeadLine><![CDATA[") if subtitle && subtitle != nil
  ...
  def mobile_preview_xml_article_info
    ...
    # @sub_head_line    = @sub_head_line.gsub("\u003E", "&gt;")
    # @sub_head_line    = @sub_head_line.gsub("\u0026", "&amp;")
    @sub_head_line    = subtitle.gsub("\r\n", "</SubTitle><SubTitle>") if subtitle && subtitle != nil
    # binding.pry
...
EOF
    ...
    three_component =<<EOF
    <TitleComponent>
      <MainTitle><%= @name_plate %> <%= @head_line %></MainTitle><% if page_number == 23 && order == 2 %>
      <% else %><% if @sub_head_line == nil && @sub_head_line == "" %><% else %>
        <SubTitle><%= @sub_head_line %></SubTitle><% end %><% end %>
    </TitleComponent>
    <ArticleComponent>
      <Content><![CDATA[<%= @data_content %>
<%= @caption %>]]>
...
EOF
    ...
    three_component =<<EOF
    <TitleComponent>
      <MainTitle><%= @name_plate %> <%= @head_line %></MainTitle><% if @sub_head_line == nil && @sub_head_line == "" %><% else %>
      <SubTitle><%= @sub_head_line %></SubTitle><% end %>
    </TitleComponent>
    <ArticleComponent>
      <Content><![CDATA[<!--[[--image1--]]//--><%= @data_content %>
  <%= @by_line %>]]>
