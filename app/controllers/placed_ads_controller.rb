class PlacedAdsController < ApplicationController
  before_action :set_placed_ad, only: [:show, :edit, :update, :destroy]

  # GET /placed_ads
  # GET /placed_ads.json
  def index
    @placed_ads = PlacedAd.all
  end
  ...
  def create
    @placed_ad = PlacedAd.new(placed_ad_params)
  ...
  private
    ...
    def set_placed_ad
      @placed_ad = PlacedAd.find(params[:id])
    end
