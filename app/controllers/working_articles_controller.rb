class WorkingArticlesController < ApplicationController
  ...
  private
    ...
    def working_article_params
      params.require(:working_article).permit(:column, :row, :order, :profile, :title, :subtitle, :body, :reporter, :email, :personal_image, :image, :quote, :name_tag, :is_front_page, :top_story, :top_position, :page_id)
    end
end
